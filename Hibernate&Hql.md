...
缺点：Hibernate鼓励使用双向一对多关联，不使用单向一对多关联。单向一对多关联映射是在one端维护关系的，必须先保存many端后才可以保存one端，所以在保存many端时该端不知道one端是否存在相应的数据，所以只能将维护的关系字段设置为null，如果为非空则无法保存。因为是one端维护关系，所以在保存one端时，会发出多余的update语句维护many端的外键关系。**性能不好，建议改为使用双向1-N关联，或者用连接表**
...
但是，如果是list属性，还需要一个字段记录序号，@OrderColumn，这时好像就要用到连接表，能不能在N端添加关联实体属性的同时再加这个序号字段呢？
...
操作：基于连接表，在两端都使用 @ManyToMany @JoinTable 显式映射连接表，指定的表名要相同，指定的外键列相互对应。如果想要某端放弃控制关联关系，在@ManyToMany中指定mappedBy属性，并去掉@JoinTable即可
